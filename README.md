# Conceitos basicos de python

- [Condições aninhadas `condicoes_aninhadas`](https://github.com/Williams25/estudos-python/tree/master/condicoes_aninhadas)
- [Estrutura de repetição `estrutura_de_repeticao`](https://github.com/Williams25/estudos-python/tree/master/estrutura_de_repeticao)
- [Tuplas `tuplas`](https://github.com/Williams25/estudos-python/tree/master/tuplas)
- [Listas `listas`](https://github.com/Williams25/estudos-python/tree/master/listas)
- [Dicionarios de dados `dicionarios_de_dados`](https://github.com/Williams25/estudos-python/tree/master/dicionarios_de_dados)
- [Funções `funcoes`](https://github.com/Williams25/estudos-python/tree/master/funcoes)
- [Modulos `modulos`](https://github.com/Williams25/estudos-python/tree/master/modulos)
- [Tratamento de erros `tratamento_de_erros`](https://github.com/Williams25/estudos-python/tree/master/tratamento_de_erros)
